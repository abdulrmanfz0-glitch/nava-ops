...
const ProfessionalReport = ({ reportData, isLoading }) => {
  ...
  // Use generated insights from state, or fallback to reportData
  const displayInsights = revenueInsights.length > 0 ? revenueInsights : (reportData.insights || []);
  const displayMetrics = reportData.metrics || [];
  const displaySummary = executiveSummary || reportData.executiveSummary || '';
  const sections = reportData.sections || [];
  const revenueRecommendations = reportData.revenueRecommendations || [];
  const costRecommendations = reportData.costRecommendations || [];
  const branchRecommendations = reportData.branchRecommendations || [];
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {displayMetrics?.slice(0, 4).map((metric, idx) => (
            <MetricBox key={idx} metric={metric} />
          ))}
        </div>

        {/* Impact Summary */}
        {reportData.totalImpact && Object.keys(reportData.totalImpact).length > 0 && (
          <ImpactSummaryBox impact={reportData.totalImpact} />
        )}
      </div>
      ...
      {displayInsights && displayInsights.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
          ...
          <div className="space-y-3">
            {displayInsights.slice(0, 5).map((insight, idx) => (
